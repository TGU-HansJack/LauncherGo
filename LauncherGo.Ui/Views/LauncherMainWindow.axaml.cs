...
public partial class LauncherMainWindow : Window
{
    ...
    private void InitializeConnectionStaticTexts()
    {
        ...
        ConnectionFrpImportButton.Content = T("导入常规", "Import Regular");
        ConnectionFrpStartButton.Content = T("启动常规", "Start Regular");
        ConnectionFrpStopButton.Content = T("停止常规", "Stop Regular");
        ConnectionThirdPartyFrpcImportButton.Content = T("导入第三方", "Import Third-party");
        ConnectionThirdPartyFrpcStartButton.Content = T("启动第三方", "Start Third-party");
        ConnectionThirdPartyFrpcStopButton.Content = T("停止第三方", "Stop Third-party");
    ...
    private async Task ImportConnectionExecutableAsync(ConnectionProcessKind kind)
    {
        var files = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
        {
            Title = kind == ConnectionProcessKind.Frp
                ? T("导入常规内网穿透可执行文件", "Import regular FRP executable")
                : T("导入第三方内网穿透可执行文件", "Import third-party FRPC executable"),
            AllowMultiple = false,
            FileTypeFilter =
            [
    ...
    private async void OnConnectionFrpStartClick(object? sender, RoutedEventArgs e)
    {
        SaveFrpSettings(updateStatus: false);
        await StartConnectionProcessAsync(ConnectionProcessKind.Frp);
    }

    private async void OnConnectionFrpStopClick(object? sender, RoutedEventArgs e)
    {
        await StopConnectionProcessAsync(ConnectionProcessKind.Frp);
    }

    private async void OnConnectionThirdPartyFrpcStartClick(object? sender, RoutedEventArgs e)
    {
        SaveFrpSettings(updateStatus: false);
        await StartConnectionProcessAsync(ConnectionProcessKind.ThirdPartyFrpc);
    }

    private async void OnConnectionThirdPartyFrpcStopClick(object? sender, RoutedEventArgs e)
    {
        await StopConnectionProcessAsync(ConnectionProcessKind.ThirdPartyFrpc);
    }
