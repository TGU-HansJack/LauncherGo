...
`.github/workflows/windows-small-package.yml` packages that output as a zip so you can distribute a smaller download than the self-contained installer. Users of this package must install `.NET 10 Runtime (x64)` before launching the app.

The same workflow also produces `LauncherGo-Small-Setup-<version>-win-x64.exe`. This is a smaller installer that does not bundle the .NET Runtime. It checks for `.NET 10 Runtime (x64)` and, when it is missing, offers a task checkbox that opens Microsoft's download page after installation. The full self-contained installer is still generated by `installer/LauncherGo.iss`.
