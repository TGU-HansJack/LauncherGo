...
The same workflow also produces `LauncherGo-Small-Setup-<version>-win-x64.exe`. This is a smaller installer that does not bundle the .NET Runtime, so `.NET 10 Desktop Runtime (x64)` is still required after installation. The full self-contained installer is still generated by `installer/LauncherGo.iss`.
