...
The same workflow also produces `LauncherGo-Small-Setup-<version>-win-x64.exe`. This is a smaller installer that does not bundle the .NET Runtime. It checks for `.NET 10 Desktop Runtime (x64)` and, when it is missing, offers a task checkbox that opens Microsoft's download page after installation. The full self-contained installer is still generated by `installer/LauncherGo.iss`.
