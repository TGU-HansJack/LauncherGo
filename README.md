...
同一工作流还会生成 `LauncherGo-Small-Setup-<version>-win-x64.exe`。这是不包含 .NET Runtime 的小体积安装器，安装后仍需要 `.NET 10 Desktop Runtime (x64)`；完整自包含安装器仍由 `installer/LauncherGo.iss` 生成。
